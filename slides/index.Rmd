...
- Eyeball the regression model. The plot shows that the relationship between x and y is basically linear.
...
- The mean of residuals is 1.136591e-14, which is almost equal to zero.
```{r}
...
- Cor.test is used to test whether predictor and residuals are correlated or not. The returned p-value is 1 and the null hypothesis cannot be rejected.
...
For model specification assumptions, the plot in (1) shows that the relationship between x and y is basically linear. 

For the error term assumptions, (2) displays that the mean of residuals is almost equal to 0; the first plot of (3) illustrates that the variance around predicted values is consistent; the second and third plot of (3) exhibit that the residuals are basically normally distributed; (4) uses cor.test to test if predictor and residuals are correlated and the returned p-value is 1, which is much larger than 0.05, thus we cannot reject the null hypotheses that true correlation is equal to 0 and conclude that predictor and residuals are not correlated.
